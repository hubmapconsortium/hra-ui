...
describe('CellSummaryState', () => {
  ...
  beforeEach(() => {
    ...
    ctx.getState.mockReturnValue({ summaries: undefined, aggregates: undefined });
  });
  ...
  describe('load(ctx, action)', () => {
    ...
    it('should load the cell summaries', () => {
      ...
      state.load(ctx, new Load(testIri));
      expect(ctx.patchState).toHaveBeenCalledWith({ aggregates: [], summaries: summaries });
    });
  });

  describe('computeAggregates', () => {
    it('should compute aggregrate data and update state', () => {
      ctx.getState.mockReturnValue({ summaries });
