...
describe('assetsUrl(path, type)', () => {
  ...
  it('returns a signal with the built url when using a path getter', () => {
    TestBed.inject(APP_ASSETS_HREF).set(base);
    const result = TestBed.runInInjectionContext(() => assetsUrl(pathGetter, 'css'));
    expect(isSignal(result)).toBeTruthy();
    expect(result()).toEqual(cssUrl);
  });
});
