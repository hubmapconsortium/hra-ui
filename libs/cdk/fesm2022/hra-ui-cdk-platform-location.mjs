...
const PATCHABLE_PLATFORM_LOCATION_HISTORY_METHODS = [
    'replaceState',
    'pushState',
    'forward',
    'back',
 ...
 */
function patchPlatformLocationHistoryMethod(impl, method) {
    const originalFn = impl[method];
    if (originalFn) {
        impl[method] = function (...args) {
            try {
 ...
 */
function patchPlatformLocationHistoryMethods(impl, methods = PATCHABLE_PLATFORM_LOCATION_HISTORY_METHODS) {
 ...
 * @returns Providers
 */
function provideNothrowPlatformLocation() {
...
//# sourceMappingURL=hra-ui-cdk-platform-location.mjs.map
