...
describe('OrganVersionComponent', () => {
  ...
  it('should create', async () => {
    await expect(shallow.render({ bind: { versionData: testVersionData, organInfo: testOrganInfo } })).resolves.toBeDefined();
  });
  ...
  describe('ngOnInit()', () => {
    it('should setVersion if headerinfo is not present', async () => {
      ...
      const { instance, inject } = await shallow.render({ bind: { organInfo: testOrganInfo } });
  ...
  describe('iCaseEquals()', () => {
    it('should check and return if 2 strings are equal after converting to lowercase', async () => {
      const { instance } = await shallow.render({ bind: { versionData: testVersionData, organInfo: testOrganInfo } });
  ...
  describe('setVersion()', () => {
    it('should set the initial organinfo if specified version is not found', async () => {
      const { instance } = await shallow.render({ bind: { organInfo: testOrganInfo } });
  ...
  describe('setOrgan()', () => {
    ...
    it('should set the title with some additional text when header data is present', async () => {
      ...
      const { instance } = await shallow.render({ bind: { organInfo: testOrganInfo, headerInfo: testHeaderInfo } });
      const testOrgan = 'test';
      instance.setOrgan(testOrgan);
