...
// Mock URL constants
jest.mock('./../static/url', () => ({
  URL: 'http://test-api.com',
  getAssetsURL: jest.fn(() => 'assets/data/1.0/test.csv'),
}));
...
describe('SheetService', () => {
  ...
  it('should fetch data from assets', () => {
    ...
    const req = httpMock.expectOne('assets/data/1.0/test.csv');
    expect(req.request.method).toBe('GET');
  ...
  it('should fetch playground data', () => {
    ...
    const req = httpMock.expectOne('http://test-api.com/v2/playground');
    expect(req.request.method).toBe('GET');
    req.flush(mockResponse);
  });
