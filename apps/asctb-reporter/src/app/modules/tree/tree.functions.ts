 ...
 *
 */
export function makeAS(data: Row[], isForReport = false, isReportNotOrganWise = false): AS[] {
  const anatomicalStructures: AS[] = [];
  let id = ST_ID;
  try {
    data.forEach((row) => {
      row.anatomical_structures.forEach((str, i) => {
        if (i === row.anatomical_structures.length - 1 || (isForReport && str.name !== 'Body')) {
 ...
 */
export function makeCellTypes(data: Row[], isForReport = false, isReportNotOrganWise = false): CT[] {
  const cellTypes: CT[] = [];
  try {
    data.forEach((row) => {
      row.cell_types.forEach((str) => {
        ...
        if (foundIndex === -1) {
          ...
          if ('isNew' in str) {
            ...
            (cellTypes[foundIndex] as unknown as { pathColor: string }).pathColor = str.color ?? '';
          }
          row.biomarkers.forEach((marker) => {
 ...
 */
export function makeBioMarkers(data: Row[], type?: string, isForReport = false, isReportNotOrganWise = false): B[] {
  const bioMarkers: B[] = [];
  try {
    data.forEach((row) => {
      ...
      currentBiomarkers.forEach((str) => {
        ...
        if (foundIndex === -1) {
          ...
          if ('isNew' in str) {
            ...
            (bioMarkers[foundIndex] as unknown as { pathColor: string }).pathColor = str.color ?? '';
          }
          if (row.cell_types.length) {
            row.cell_types.forEach((cell) => {
